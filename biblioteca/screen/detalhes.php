    ...
    if (isset($_GET['id'])) {
        ...
        if ($result->num_rows > 0) {
            ...
            if ($livro["disponivel"] == 0) {
                echo "<span class='erro'>Livro esgotado!</span>";
            } else {
                echo "<a href='livros_reservados.php?id=" . $livroId . "' class='benn' style='position: relative; top: 610px;'>Alugar</a>";
            }

            if (isset($_SESSION['livro_reservado'])) {
                if ($_SESSION['livro_reservado']) {
                    echo "<span class='sucesso'>Livro reservado com sucesso!</span>";
                } else {
                    echo "<span class='erro'>Livro já reservado!</span>";
                }
                unset($_SESSION['livro_reservado']); // Limpar a variável de sessão
            }
